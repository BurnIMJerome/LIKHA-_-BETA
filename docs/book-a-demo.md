---
layout: default
title: Book a Demo
description: Request a Likha demonstration covering Agentic AI, RPA, document processing, integrations, and deployment options.
page_class: marketing-page
---
{% assign docs_root = site.docs_root | default: '/docs' %}

<section class="marketing-page-hero">
  <div class="section-shell">
    <p class="eyebrow">Book a Demo</p>
    <h1>See how Likha fits your automation work</h1>
    <p>A Likha demonstration can be tailored around the workflows, applications, documents, approvals, integrations, and deployment model that matter to your team.</p>
  </div>
</section>

<section class="marketing-page-body">
  <div class="section-shell">
    <div class="section-heading">
      <p class="section-kicker">What we can demonstrate</p>
      <h2>From intelligent decisions to reliable execution</h2>
      <p>Explore how Likha brings Agentic AI and low-code automation together in one practical platform.</p>
    </div>

    <div class="marketing-card-grid">
      <article class="marketing-card"><h2>Agentic AI workflows</h2><p>Agents that reason, use approved flow tools, work with knowledge, and collaborate with specialists.</p></article>
      <article class="marketing-card"><h2>Desktop and browser automation</h2><p>Visual workflows that interact with Windows applications and modern web experiences.</p></article>
      <article class="marketing-card"><h2>Documents and OCR</h2><p>AI-powered document processing, field extraction, vision, tables, and OCR activities.</p></article>
      <article class="marketing-card"><h2>Human-in-the-loop approvals</h2><p>Operational controls for work that needs review or approval before execution continues.</p></article>
      <article class="marketing-card"><h2>Business integrations</h2><p>Connect APIs, databases, email, files, scripts, desktop apps, browser apps, and third-party systems.</p></article>
      <article class="marketing-card"><h2>Deployment options</h2><p>Discuss local development, on-premises operation, private infrastructure, and cloud-connected environments.</p></article>
    </div>

    <div class="contact-panel">
      <div>
        <h2>Request a Likha demonstration</h2>
        <p>Email the project owner at <a href="mailto:Jearomev@yahoo.com">Jearomev@yahoo.com</a> with a short description of the process you would like to explore.</p>
      </div>
      <div class="hero-actions">
        <a class="button button-primary" href="mailto:Jearomev@yahoo.com?subject=Likha%20Demo%20Request">Email the project owner</a>
        <a class="button button-secondary" href="{{ docs_root | append: '/README.html' | relative_url }}">Explore Documentation</a>
      </div>
    </div>
  </div>
</section>

